<?php
    ...
    $search_crawler_authenticated = check_http_basic_auth(getenv('SEARCH_INSIDE_WP_API_HASHED_TOKEN'));
    if ( ! (is_user_logged_in() || $search_crawler_authenticated) ) {
    ...
    function check_http_basic_auth ($htpasswd_line) {
        if (! $htpasswd_line) {
            return false;
        }
        ...
        if (str_starts_with($authHeader, 'Basic ')) {
            ...
            if (password_verify($decodedCredentials, $htpasswd_line)) {
                return true;
            }
        }
        return false;
